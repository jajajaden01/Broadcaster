...
const { location, title } = IncidentFakeData.saveRedFlag();

describe('Testing an endpoint for updating Red-Flag location', () => {
  before((done) => {
    chai.request(app)
  ...
  it('should return 403 http status code on bad request and missed token', (done) => {
    chai.request(app)
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.have.property('error').equals('"location" is required').that.is.a('string');
        return done();
      });
  });

  it('should return 403 http status code on empty request about location', (done) => {
    chai.request(app)
      .patch(`/api/v1/red-flags/${1}/location`)
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.be.an('object');
        expect(res.body).to.have.property('status').equals(400).that.is.a('number');
        expect(res.body).to.have.property('error').equals('"location" is not allowed to be empty').that.is.a('string');
        return done();
      });
  });
  ...
  it('should return 404 http status code on No found record to update it location', (done) => {
    chai.request(app)
      .patch(`/api/v1/red-flags/${1}/location`)
      .set('token', user1Token)
      .send({ location: String(location) })
      .end((err, res) => {
        ...
        expect(res.body).to.have.property('data').equals('Sorry! a red-flag\'s location to edit, not found.').that.is.a('string');
        return done();
      });
  });
  ...
  it('should return 400 http status code on the request that not allowed', (done) => {
    chai.request(app)
      .patch(`/api/v1/red-flags/${1}/location`)
      .set('token', user1Token)
      .send({ location: String(location), title })
      .end((err, res) => {
        if (err) return done(err);
