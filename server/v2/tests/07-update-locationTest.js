...
let insertedData = {};
let insertedId = 0;
describe('TEST 07: Testing an endpoint for updating Red-Flag location', () => {
  ...
  it('should return 200 http status code on success request', async () => {
    try {
      const res = await chai.request(app)
        .patch(`/api/v2/red-flags/${insertedId}/location`)
        .set('token', user1Token)
        .send({ lat: redFlag.editLat, long: redFlag.editLong });
      ...
      expect(res.body).to.have.property('data').that.includes.property('message').equals('Updated red-flag record\'s location.').that.is.a('string');
    } catch (err) {
  ...
  it('should return 404 http status code on No found record to update it location', async () => {
    try {
      const res = await chai.request(app)
        .patch(`/api/v2/red-flags/${0}/location`)
        .set('token', user1Token)
        .send({ lat: redFlag.editLat, long: redFlag.editLong });
