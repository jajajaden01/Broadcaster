...
describe('TEST 10: Testing an endpoint for updating Red-Flag status as a user, put a incident in Pending', () => {
  it('should return 201 http status code on success. after creating the 2nd record', async () => {
    const res = await chai.request(app)
      ...
      .field('status', redFlag.draftStatus)
  ...
  it('should return 403 http status code on unallowed access', async () => {
    try {
      const res = await chai.request(app)
        .patch(`/api/v2/red-flags/${insertedId}/pending`)
  ...
  it('should return 200 http status code on updated status about put in Pending an incident', async () => {
    try {
      const res = await chai.request(app)
        .patch(`/api/v2/red-flags/${insertedId}/pending`)
        .set('token', user1Token);
  ...
  it('should return 404 http status code on No found record to update it status', async () => {
    try {
      const res = await chai.request(app)
        .patch(`/api/v2/red-flags/${0}/restart`)
        .set('token', user1Token);
  ...
  it('should return 404 http status code on endpoint not found', async () => {
    const res = await chai.request(app)
      .patch(`/api/sdfsdf/v2/red-flags/${insertedId}/pending`)
      .set('token', 'bad-token');
